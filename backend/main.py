...
app = FastAPI(
    ...
    description="High-Performance, Cyber-Secured FastAPI Backend for Portfolio with Automated Database Logging",
    version="1.0.0",
    lifespan=lifespan
)
...
# 2. Cyber Security Headers & Automated Request Database Logging Middleware
@app.middleware("http")
async def add_cyber_security_headers_and_db_logging(request: Request, call_next):
    ...
    # Save request & message telemetry in Database Automation (skip docs & static noise)
    if engine and not request.url.path.startswith(("/docs", "/openapi.json", "/favicon.ico", "/redoc")):
        try:
            ...
            print(f"Notice: Automated DB Request logging deferred: {err}")
