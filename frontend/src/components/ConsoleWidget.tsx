...
export const ConsoleWidget = () => {
  ...
  // Handle AI Chat Submit with Database Automation
  const handleChatSubmit = async (e?: React.FormEvent, customQuery?: string) => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/nlp/chat`, {
        method: "POST",
      ...
      console.warn("Backend API unavailable, using local chat fallback", err);
    }
